...
(i.e. for Mm10.1.bt2 etc.: /path/to/index/Mm10), followed by [ENTER] : " refGen
...
# Number of alignment per read
read -p "Type the number of aln per read you want to allow for Bowtie2, followed by [ENTER] : " aln
until [[ "$aln" =~ ^[1-9]+ ]]
do
    echo " $ERROR : The number of aln per read must be an integer greater than 0 !"
    read -p "Type the number of aln per read you want to allow for Bowtie2, followed by [ENTER] : " aln
done
...
then
    ...
    do
        ...
        bowtie2 -p $threads -k $aln -x $refGen -1 $forward  -2 $inputFolder/$reverse \
        | samtools view -F 4 -bS - > $outputFolder/$prefix"_aln.bam"
        
        # Sorting
        echo "Sorting file $prefix""_aln.bam by chromosomal coordinates..."
        samtools sort $outputFolder/$prefix"_aln.bam" $outputFolder/$prefix"_sorted-k$aln"
        echo "Alignment file $prefix""_sorted-k$aln.bam created !"
        
        # Indexing
        echo "Creating index for file $prefix""_sorted-k$aln.bam..."
        samtools index $outputFolder/$prefix"_sorted-k$aln.bam"
        echo "Index file $prefix""_sorted-k$aln.bam.bai created !"
...
then
    ...
    do
        ...
        bowtie2 -p $threads -k $aln -x $refGen -U $file | samtools view -F 4 -bS - \
...
echo "You can now visually inspect your data, do the normalization or do the peakcalling with MACS2 ! :)"
